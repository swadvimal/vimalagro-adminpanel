  ...
  const validate = () => {
    ...
    formData.category.forEach((cat, i) => {
      if (!cat.categoryName.trim())
        err[`categoryName_${i}`] = "Category Name is required";
      if (!cat.productName.trim())
        err[`productName_${i}`] = "Product Name is required";
      if (!cat.displayOrder.trim())
        err[`displayOrder_${i}`] = "Display Order is required";
      if (!cat.description.trim())
        err[`description_${i}`] = "Description is required";
      if (!cat.categoryBanner && !editingId)
        err[`categoryBanner_${i}`] = "Banner is required";
    });
    return err;
  };
    ...
    try {
      ...
      formData.category.forEach((cat, i) => {
        ...
        if (cat.categoryBanner)
          fd.append(`categoryBanner_${i}`, cat.categoryBanner);
      });
        ...
        <div className="rounded-3 shadow overflow-hidden">
          ...
          >
            <h6 className="fw-bold m-0 text-dark">
              ...
              {editingId ? "Edit Categories" : "Add Categories"}
            </h6>
          </div>
              ...
              <div key={index} className="border p-3 my-2 rounded-3">
                ...
                <div className="mt-2">
                  <label className="d-block fw-bold">
                    Category Banner (Rectangle Shape)
                  </label>
                  <input
                    type="file"
                  ...
                  {imagePreviews[index] && (
                    <img
                      ...
                      width={60}
                      height={60}
                      className="mt-2 object-fit-fill"
                    />
                  )}
                </div>
                <div className="mt-2">
          ...
          {fetching ? (
            ...
            <table className="table table-bordered border-secondary custom-table table-hover text-center">
              ...
              <tbody className="pera">
                {categoriesData.map((catDoc) =>
                  catDoc.category.map((cat, i) => (
                    <tr key={i}>
                      <td>
                        {cat.categoryBanner && (
                          <img src={cat.categoryBanner} alt="" width="50" />
                        )}
                      </td>
                      ...
                      <td>
                        <div className="d-flex flex-column flex-md-row flex-lg-row justify-content-center align-items-center">
                          <FaEdit
                            ...
                            onClick={() => handleEdit(catDoc)}
                            style={{ cursor: "pointer" }}
                          />
                          <FaTrash
                            className="text-danger fs-5"
                            onClick={() => handleDelete(catDoc._id)}
                            style={{ cursor: "pointer" }}
                          />
                        </div>
                      </td>
                ...
                categoriesData.every((doc) => doc.category.length === 0) ? (
                  <tr>
